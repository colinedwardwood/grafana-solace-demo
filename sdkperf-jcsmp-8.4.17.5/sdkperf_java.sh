...
# Allow the user to give custom JVM args such as JFR or proxies
[ -z $SOLACE_SDKPERF_CUSTOM_VM_ARGS ] || SOLACE_VM_ARGS="$SOLACE_VM_ARGS $SOLACE_SDKPERF_CUSTOM_VM_ARGS"
...
if echo "$*" | grep -q ".*-as=kerberos.*"; then
    [ -z $SOLACE_SDKPERF_JAAS_CONF ] && \
...
# Check for any thirdparty API sdkperf clients.
for FILE in ./lib/thirdparty/*.jar; do
...
CLASSPATH="$CLASSPATH" java $SOLACE_VM_ARGS com.solacesystems.pubsub.sdkperf.SDKPerf_java "$@"
